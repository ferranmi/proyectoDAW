    ...
    @foreach ($news as $new)
        ...
        <div class="col-lg-5 mt-3 d-flex justify-content-center align-items-center mx-auto" >
            <div class="row featurette dissenyBorders">
                <div class=" col-lg-7  fondos_targetas " >
                    <h2 class="featurette-heading"> <a href="noticias/{{ $new->code }}">
                            titulo: {{ $new->title }}
                        </a></h2>
                    <span class="text-muted"> contenido: {{ $new->d_short }} </span>
                </div>
                <div class="col-lg-5 col-md-5 d-lg-block d-none p-0 m-0"> <img style="height: 200px; width: 100%;" src="{{ Storage::url($new->image) }}"> </div>
            </div>
        </div>
    @endforeach
